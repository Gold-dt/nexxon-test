...
<head>
    ...
    <title>Toplay - Mutes</title>
	<link href="favicon.ico" rel="shortcut icon" type="image/x-icon" >
	<meta name='robots' content='index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' />
	<meta name="description" content="List the players who have been muted on the server." />
...
$(document).ready(function(){
    jQuery(".cell1").click(function() {
		...
		var data_type = jQuery(this).attr("data-type") || 'Unknown';
        ...
        Swal.fire({
            icon: "info",
            html: steamProfileLink + '<b>Admin:</b> ' + (data_adminname !== 'Unknown' ? data_adminname : 'Unknown') + '<br><b>Reason:</b> ' + (data_reason !== 'Unknown' ? data_reason : 'Unknown') + '<br><b>Type:</b> ' + (data_type !== 'Unknown' ? data_type : 'Unknown'),
...
function setActiveButton(serverName) {
    ...
    <h1>List the players who have been muted on the server.</h1>
...
if (!empty($search)) {
    ...
    $query = "SELECT type, player_steamid, player_name, admin_steamid, admin_name, reason, duration, ends, created, status FROM {$prefix}sa_mutes WHERE player_name LIKE :search ORDER BY created DESC LIMIT :offset, :limit";
} else {
    // If there is no search value, include null values for "player_name."
    $query = "SELECT type, player_steamid, player_name, admin_steamid, admin_name, reason, duration, ends, created, status FROM {$prefix}sa_mutes ORDER BY created DESC LIMIT :offset, :limit";
}
...
// Calculate the total number of records (not just those on the current page)
        $countQuery = "SELECT COUNT(*) as total FROM {$prefix}sa_mutes WHERE player_name LIKE :search";
...
$playerName = ($row["player_name"] !== null && $row["player_name"] !== '') ? $row["player_name"] : 'Unknown';
$adminName = ($row["admin_name"] !== null && $row["admin_name"] !== '') ? $row["admin_name"] : 'Unknown';
$reason = ($row["reason"] !== null && $row["reason"] !== '') ? $row["reason"] : 'Unknown';
...
$ends = ($row["ends"] !== null && $row["ends"] !== '') ? $row["ends"] : 'Unknown';
$created = ($row["created"] !== null && $row["created"] !== '') ? $row["created"] : 'Unknown';

  echo '<div class="row">
            <div class="cell" data-title="#">' . $startRowNumber . '</div>
            <div class="cell cell1" data-title="Name" data-playersteamid="' . $row["player_steamid"] . '" data-adminname="' . $row["admin_name"] . '" data-type="' . $row["type"] . '" data-playername="' . $row["player_name"] . '" data-adminsteamid="' . $row["admin_steamid"] . '" data-reason="' . $row["reason"] . '" data-duration="' . $row["duration"] . '" data-ends="' . $row["ends"] . '" data-created="' . $row["created"] . '" data-status="' . $row["status"] . '">' . $playerName . '</div>
